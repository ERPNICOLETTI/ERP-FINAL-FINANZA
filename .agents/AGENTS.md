...
- OCR inicial 2026-09-09: `core_sistema/ocr_documentos.py` (Tesseract local spa+eng, dos pasadas, todas las páginas, cajas/confianza y QR zxing-cpp), `modulo_compras/facturas_scan_parser.py` (QR candidato validado), `lector_facturas_scan.py` (CLI ELT). Original inmutable + RAW antes del motor; resultados versionados en `compras_scan_extracciones`, sin modificar facturas fiscales ni sus vínculos. Lote `Desktop/Facturas inbox`: 28 PDF / 31 páginas, versión 1.1 leída; 13 páginas coinciden por QR/clave/fecha/total con DB, 18 requieren revisión. Vista `/compras/ocr` y API `/api/compras/ocr`. No confundir LEIDO con validado. Pendiente: extracción estructurada fiable sin QR, control OCR-versus-QR, rotación/recortes, confirmación visual y vínculo por página (los multipágina pueden contener facturas distintas). No hay archivado automático ni ingesta web OCR todavía. El conversor legacy del core sigue separado y limitado: no usar master global para este lote. Ningún escaneo original se eliminó.
